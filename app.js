...
App({
  ...
  onLaunch(e) {
     ...
     this._post_form("user.balancelog/suoding",{},(res)=>{
       console.log("onlaunch的调用的方法"+JSON.stringify(res))
       if(res.code == 0){
        wx.showModal({
          title: '提示',
          content: res.data,
    ...
    console.log("检查更新返回的值"+this.checkIsLogin())
    console.log("小程序启动场景" +JSON.stringify(e));
  ...
  onStartupScene(query) {
    console.log("启动场景",JSON.stringify(query));
    // 获取场景值
    let scene = this.getSceneData(query);
    console.log("扫码",query);
    // 记录推荐人id
    let refereeId = query.referee_id ? query.referee_id : scene.uid;
    let register_code = query.register_code ? query.register_code : scene.register_code;
    refereeId > 0 && (this.saveRefereeId(refereeId));
    register_code !== '' && (this.saveregister_code(register_code));
  },
  adduser() {
    let _this = this;
    _this._get("live.room/adduser", {
    }, result => {
  ...
  saveregister_code(register_code) {
    if (!wx.getStorageSync('register_code'))
  ...
  getUserInfo2(e, callback) {
    ...
    wx.login({
      success(res) {
        // 发送用户信息
        console.log("授权登录");
        console.log("login登录返回的res"+JSON.stringify(res));
        App._post_form('user/login', {
          ...
          referee_id: wx.getStorageSync('register_code'),
          mobile: ''
        }, result => {
          // 记录token user_id
          console.log("res111111111111111111111"+JSON.stringify(result))
